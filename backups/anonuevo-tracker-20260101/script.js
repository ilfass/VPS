...
// Calcular la posición del sol y el terminador (línea día/noche)
function getSunPosition(date) {
    ...
    // Longitud del sol (donde es mediodía)
    var utcHours = date.getUTCHours() + date.getUTCMinutes() / 60 + date.getUTCSeconds() / 3600;
    var sunLon = 180 - (utcHours * 15);

    // Normalizar
    while (sunLon > 180) sunLon -= 360;
    while (sunLon < -180) sunLon += 360;

    return { dec: declination, lon: sunLon };
}

function getTerminatorCoordinates(date) {
    const sunPos = getSunPosition(date);
    const sunDec = sunPos.dec;
    const sunLon = sunPos.lon;
    const coordinates = [];

    // Generar puntos del terminador de -180 a 180 grados
    for (let lon = -180; lon <= 180; lon += 2) {
        let lat;
        // Fórmula: tan(lat) = -cos(lon - sunLon) / tan(dec)
        const deltaLon = (lon - sunLon) * Math.PI / 180;
        const tanDec = Math.tan(sunDec * Math.PI / 180);

        if (Math.abs(tanDec) < 1e-9) {
            // Equinoccio (sol en ecuador)
    ...
    // Cerrar el polígono para cubrir la zona nocturna
    // Si el sol está en el Norte (Dec > 0), la noche está en el Sur. Cerrar por -90.
    // Si el sol está en el Sur (Dec < 0), la noche está en el Norte. Cerrar por 90.
    const closeLat = sunDec > 0 ? -90 : 90;

    coordinates.push([180, closeLat]);
    coordinates.push([-180, closeLat]);
    coordinates.push(coordinates[0]); // Cerrar loop
...
function updateHighmapsDayNight() {
    ...
    try {
        ...
        // Calcular coordenadas del polígono de noche
        const coordinates = getTerminatorCoordinates(now);

        // Buscar o crear la serie de noche
        let nightSeries = state.highmapsChart.series.find(s => s.name === 'Night');

        if (!nightSeries) {
            console.log('🌑 Creando capa de noche...');
            state.highmapsChart.addSeries({
                name: 'Night',
                type: 'map',
                color: 'rgba(0, 0, 0, 0.65)', // Sombra más oscura
        ...
        // Eliminar el div antiguo si existe (limpieza)
        const oldShadow = document.getElementById('dayNightShadow');
        if (oldShadow) oldShadow.remove();

        // Forzar actualización de clones para el efecto de scroll infinito
        // Eliminamos los clones para que updateMapRotation los recree actualizados
        // Solo hacerlo si la posición del mapa ha cambiado significativamente para evitar parpadeo
        if (now.getSeconds() % 5 === 0) {
            ...
            if (cloneRight) cloneRight.remove();
            if (cloneLeft) cloneLeft.remove();
            state.mapDuplicated = false;
        }
